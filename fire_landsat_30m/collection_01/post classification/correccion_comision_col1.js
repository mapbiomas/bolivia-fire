/** 
 * Script para la correción de áreas de comisión
 * 
 * MapBiomas Fuego Bolivia 2026 
 * Collection 1 
 * 
 * MapBiomas Bolivia 
**/  
...
// PARAMETROS DEL REMAP

// Cambiar unicmente las lineas 9, 11, 12 y seleccionar el activar el remap del año de interes desde la fila 16 ***
var param = {
  pais : 'bolivia', // bolivia
  sensor : 'l89', // Para los años 1995 a 1998 = l5 -  Para los años 1999 a 2012 = l57
                  // Para los años 2013 a 2021 = l78 - Para los años 2022 a 2024 = l89 
  region : 10,    // Seleccionar la region 1 a 44 ***
  year : 2025,    // Seleccionar el año entre 1995 a 2025 ***
  version_input : 1,  //collection1_fire_mask_v1
  version_output : 2, //collection1_fire_mask_v2
  remapGeometry: [  //Seleccionar la geometria con el año que corresponda ***
                     remap_1995,
                    // remap_1996,
                    // remap_1997,
                    // remap_1998,
  ...
  Graficar: {
      graph: true, //poner false para que no se genere el grafico
      claseGraph: 3, //clase de la cual se haran las estadisticas
      scale: 120 //30, 60, 90, 120, 250
  },
};
...
var carpeta = 'projects/mapbiomas-bolivia/assets/FIRE/COLLECTION1/CLASSIFICATION_COLLECTIONS'
var path = carpeta+'/collection1_fire_mask_v'+param.version_input
var filename = 'burned_area_bolivia_'+param.sensor+'_region'+param.region+'_'+param.year
var collection = ee.ImageCollection(path)
var imagefuego = ee.Image(path+'/'+filename)

print('coleccion de imagenes',collection)
print('asset:',filename)
print('imagen:',imagefuego)
    ...
    polygons.forEach(function( polygon ) {
      ...
      var excluded = polygon.map(function( layer ){
        ...
        var imageOperation = function(year, previousImage) {
          var remapYear = area.select(ee.String('monthOfYear'))
                              .remap(from, to)
                              .rename(ee.String('monthOfYear'));
      ...
      //Map.addLayer(excluded, {}, 'excluded '+param.year, false);
...
var geomremap = filtered.eq(13); // Booleano
var finalImage = imagefuego.updateMask(geomremap.not())

//visualización de capas
Map.addLayer(imagefuego, {"min":1,"max":13,"palette":["ff0202","5904ff"]}, 'Clasificacion con filtros '+param.year, true)
//Map.centerObject(imagefuego, 15)
//Map.addLayer(filtered, {"min":1,"max":13,"palette":["ff0202","5904ff"]}, 'filtered '+param.year, false)
Map.addLayer(geomremap.selfMask(), {min:0, max:1, palette: 'orange'}, 'Areas borradas '+param.year, true)
Map.addLayer(finalImage, {"min":1,"max":13,"palette":["ff0202","5904ff"]}, 'Clasificacion final '+param.year, true)

// EXPORTS 
  Export.image.toAsset({
      'image': finalImage,
      'description': filename+'_rm',
      'assetId': carpeta+'/collection1_fire_mask_v'+param.version_output+'/'+filename+'_rm' ,
      'pyramidingPolicy': {
          '.default': 'mode'
      },
      'region': imagefuego.geometry().bounds(),
      'scale': 30,
      'maxPixels': 1e13
  });
...
/** @description Toolkit for collection and evaluation of collections used in classifications of collection 2 of MapBiomas-Fogo.
  ...
  * reg
   ...
   Toolkit development: @IPAM - Brasília, DF - BR
     - Wallace Silva, Vera Laísa and Dhemerson Cociane
...
var year = 1995;
var table = 'Regiones Bolivia';//'Regiones Peru';
var region = 'bolivia_r6_fan';//'peru_r5_costa';
...
var options = {
  ...
  folder_toAsset:'projects/mapbiomas-bolivia/assets/FIRE/COLLECTION1/SAMPLES/',
...
options.dataset = [
  ...
  },
  {
    ...
    allProcess:function(col){
      col = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2').filter(ee.Filter.inList('system:index', blockList_landsat).not())
        .map(function(image){
          image = clipBoard_Landsat (image);
          image = corrections_LS8_col2(image);
          image = addBand_NBR(image);
      ...
      var merge = ee.ImageCollection('LANDSAT/LE07/C02/T1_L2').filter(ee.Filter.inList('system:index', blockList_landsat).not())
        .map(function(image){
          image = clipBoard_Landsat (image);
          image = corrections_LS57_col2(image);
          image = addBand_NBR(image);
  ...
  },
  {
    ...
    allProcess:function(col){
      ...
      var l7 = ee.ImageCollection('LANDSAT/LE07/C02/T1_L2').filter(ee.Filter.inList('system:index', blockList_landsat).not())
        .map(function(image){
          image = clipBoard_Landsat (image);
          image = corrections_LS57_col2(image);
          image = addBand_NBR(image);
      ...
      var l9 = ee.ImageCollection('LANDSAT/LC09/C02/T1_L2').filter(ee.Filter.inList('system:index', blockList_landsat).not())
        .map(function(image){
          image = clipBoard_Landsat (image);
          image = corrections_LS8_col2(image);
          image = addBand_NBR(image);
  ...
  //MOSAICOS CLASSIFICADO
  {
    ...
    'name':'landsat5_C02',
    'outputName':'l5',
    'outputFolder':'mosaicos_to_classify_col3/',
    'shown':false,
  ...
  },
  {
    ...
    allProcess:function(col){
      col = ee.ImageCollection('LANDSAT/LT05/C02/T1_L2').filter(ee.Filter.inList('system:index', blockList_landsat).not())
        .map(function(image){
          image = clipBoard_Landsat (image);
          image = corrections_LS57_col2(image);
          image = addBand_NBR(image);
  ...
  },
  {
    ...
    allProcess:function(col){
      ...
      var merge = ee.ImageCollection('LANDSAT/LC09/C02/T1_L2')
      .filter(ee.Filter.inList('system:index', blockList_landsat).not())
        .map(function(image){
          image = clipBoard_Landsat (image);
          image = corrections_LS8_col2(image);
...
/*  
  ...
  }, 
  {
    ...
    reduceProcess:function(col,month){
      ...
      col = ee.ImageCollection(col).map(function(image){
        // image = clipBoard_Landsat (image);
        image = corrections_sentinel_pluss(image);
        image = addBand_NBR(image);
        image = image.updateMask(buffer);
        return image;
      });
      
      col = complementar_masks_monitor(col);
...
//MOSAIC VISUALIZATION
  {
    ...
    //'id':'projects/mapbiomas-mosaics/assets/LANDSAT/FIRE/mosaics-1',
    'name':'landsat_visual',
...
/*  
  {
    ...
    allProcess:function(col){
      ...
      // image = clipBoard_Landsat (image);
      image = corrections_sentinel_beta(image);
      image = addBand_NBR(image);
      return image;
    });
    },
...
//*/
 ...
 /* {
  ...
  // --- fire scar image collections from reference projects
  {
    ...
    'years':[2000,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025],
    'vis':{
        min:0,
        max:1,
        palette:['fc6000'],
 ...
 /* {
    'dataset':'reference_data',   
    'name':'ref: Colombia Burned Area',
    'id':'projects/mapbiomas-colombia/assets/FIRE/AUXILIARY_DATA/REFERENCES/cicatrizes_fuego_reference_2017_2025',
    ...
    allProcess:function(col){
      ...
      years.forEach(function(year){
        months.forEach(function(month){
          ...
          var img = ee.Image("projects/mapbiomas-colombia/assets/FIRE/AUXILIARY_DATA/REFERENCES/cicatrizes_fuego_reference_2017_2025/cicatriz_fuego_"+year);
    ...
    allProcess:function(col){
      ...
      years.forEach(function(year){
        months.forEach(function(month){
          var st = ee.Date(''+year+'-'+month+'-01');
          var ed = st.advance(1,'month');
          var img = ee.Image("projects/ee-armandorodriguezmontellano/assets/BALATAM/AQS2_LATAM_"+year);
          
          img = img.updateMask(img.eq(month))
            .set({
              'year':year,
              'month':month,
              'system:time_start':st.millis(),
...
/*  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-suriname/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_suriname_v1',
    'name':'MapB Suriname v1',
    'shown':false,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-suriname/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_suriname_v2',
    'name':'MapB Suriname v2',
    'shown':false,
    'years':[2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024],
    'vis':{
        min:1,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-bolivia/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_bolivia_v1',
    'name':'MapB Fuego Bolivia v1',
    'shown':false,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-bolivia/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_bolivia_v2',
    'name':'MapB Fuego Bolivia v2',
    'shown':false,
    'years':[1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025],
    'vis':{
        min:1,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-bolivia/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_bolivia_v3',
    'name':'MapB Fuego Bolivia v3',
    'shown':false,
    'years':[1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025],
    'vis':{
        min:1,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-bolivia/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_bolivia_v4',
    'name':'MapB Fuego Bolivia v4',
    'shown':false,
    'years':[1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025],
    'vis':{
        min:1,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-bolivia/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_bolivia_v5',
    'name':'MapB Fuego Bolivia v5',
    'shown':false,
    'years':[1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025],
    'vis':{
        min:1,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-bolivia/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_bolivia_v6',
    'name':'MapB Fuego Bolivia v6',
    'shown':false,
    'years':[1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025],
    'vis':{
        min:1,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-bolivia/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_bolivia_v7',
    'name':'MapB Fuego Bolivia v7',
    'shown':false,
    'years':[1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025],
    'vis':{
        min:1,
 ...
 /* {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-colombia/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_colombia_v1',
    'name':'MapB Fuego Colombia v1',
    'shown':false,
    'years':[2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025],
    'vis':{
        min:1,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-chile/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_chile_v1',
    'name':'MapB Fuego Chile v1',
    'shown':false,
    'years':[2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025],
    'vis':{
        min:1,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-colombia/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_ecuador_v1',
    'name':'MapB Fuego Ecuador v1',
    'shown':false,
    'years':[2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025],
    'vis':{
        min:1,
...
/*  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-guyana/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_guyana_v1',
    'name':'MapB Guyana v1',
    'shown':false,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-paraguay/assets/FIRE/COLLECTION1/CLASSIFICATION_COLLECTIONS/collection1_fire_mask_v1',
    'name':'MapB Fuego Paraguay Col.1',
    'shown':false,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-peru/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_peru_v1',
    'name':'MapB Fuego Peru v1',
    'shown':false,
    'years':[2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024],
    'vis':{
        min:1,
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-venezuela/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_venezuela_v1',
    'name':'MapB Fuego Venezuela v1',
    'shown':false,
    'years':[2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025],
    'vis':{
        min:1,
...
//*/
  ...
  {
    'dataset':'cobertura_y_uso_de_la_tierra',
    'id':'projects/mapbiomas-public/assets/bolivia/lulc/collection3/mapbiomas_bolivia_collection3_integration_v1',
    'name':'MapB Cobertura y uso Bolivia',
    // 'shown':false,
    'years':[1985,1986,1987,1988,1989,1990,1991,1992,1993,1994,1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,
       2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024],
    'vis':{
    ...
    allProcess:function(image){
      ...
      var landcover_bolivia  = ee.Image('projects/mapbiomas-public/assets/bolivia/lulc/collection3/mapbiomas_bolivia_collection3_integration_v1');
      //landcover_bolivia = landcover_bolivia.addBands(landcover_bolivia.select(['classification_2023'],['classification_2024']));
      image = landcover_bolivia;
      var col = image.byte()
        .bandNames()
        .iterate(function(current,previous){
          current = ee.String(current);
  ...
  /*{
    ...
    'name':'MapB Cobertura y uso Chile',
    // 'shown':false,
    'years':[
    ...
    allProcess:function(image){
      ...
      ////////////////////////////
      var landcover_chile     = ee.Image(image);
      
      landcover_chile = landcover_chile
        .addBands(landcover_chile.select(['classification_2022'],['classification_2023']))
        .addBands(landcover_chile.select(['classification_2022'],['classification_2024']));

      ////////////////////////////
      image = landcover_chile;
      var col = image.byte()
        .bandNames()
        .iterate(function(current,previous){
          current = ee.String(current);
  ...
  },
  {
    'dataset':'cobertura_y_uso_de_la_tierra',
    'id':'projects/mapbiomas-public/assets/chile/collection1/mapbiomas_chile_collection1_integration_v1',
    'name':'MapB Cobertura y uso Colombia',
    // 'shown':false,
    'years':[
      // 1985,1986,1987,1988,1989,1990,1991,1992,1993,1994,1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,
      2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024],
  ...
  },
  {
    'dataset':'cobertura_y_uso_de_la_tierra',
    'id':'projects/mapbiomas-public/assets/chile/collection1/mapbiomas_chile_collection1_integration_v1',
    'name':'MapB Cobertura y uso Paraguay',
    // 'shown':false,
    'years':[
      // 1985,1986,1987,1988,1989,1990,1991,1992,1993,1994,1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,
      2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024],
...
/*  {
 ...
 /* {
  ...
  // --- dados vetoriais dinamicos (no tempo)
  {
    'dataset':'Hotspots', 
    'name':'Hotspots',
    'id':'projects/workspace-ipam/assets/FOGO/monthly-focus-sul-america',
...
function returnThumbnail (name, image,vis,shown){
  ...
  var exp_mosaic = ui.Button({
    ...
    onClick:function(){
      ...
      var folder = 'sudamerica/' + country + '/collection1/mosaics_col1/';
...
function setSubtitle (){
  ...
  setNewSubtitle('mapbiomas_col1_versions');
  options.dataset
    .filter(function(obj){return obj.dataset === 'mapbiomas_col1_versions'})
    .map(function(obj){
...
function setFocosVector (){
    ...
    function layer (){
      ...
      var name = obj.name + '-' + options.year;
      var feats_list = ee.data.listAssets('projects/workspace-ipam/assets/FOGO/monthly-focus-sul-america')
            .assets
  ...
  var samples_b = [
    {
      ...
      var select = ui.Select({
          ...
          // placeholder:'null',
          value:'projects/mapbiomas-bolivia/assets/FIRE/COLLECTION1/SAMPLES',
          onChange:function(value){
  ...
  return ui.Button({
    ...
    onClick:function(){
      ...
      var folders = {
        ...
        peru:{
          bucket:'sudamerica/peru/collection2/training_samples/',
          asset:'projects/mapbiomas-peru/assets/FIRE/COLLECTION2/SAMPLES/'          
        },
        guyana:{
